...
class SelectionPopup extends PopupPanel implements View.OnClickListener {
	...
	public void move(int selectionStartY, int selectionEndY) {
		...
		if(mTranslateButton == null) {
			mTranslateButton = (ImageView) myWindow.findViewById(R.id.selection_panel_translate);
		}
		int pictureHeight = mTranslateButton.getHeight();
		int pxof16dp = dip2px(myWindow.getContext(), 74);
...
//		layoutParams.addRule(verticalPosition);
		...
		myWindow.getHeight();
		final int screenHeight = ((View)myWindow.getParent()).getHeight();
		final int floatWindowHeight = pictureHeight + pxof16dp;
		final int margin = 10;
		int topDist = screenHeight - selectionEndY;
		if(selectionStartY > floatWindowHeight + margin){
			layoutParams.topMargin = selectionStartY - floatWindowHeight - margin;
		}else {
			layoutParams.topMargin = selectionEndY + margin;
		}
	}
